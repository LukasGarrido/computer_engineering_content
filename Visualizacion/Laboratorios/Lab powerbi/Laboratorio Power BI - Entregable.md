...
`order_details` es la tabla de hechos (muchas filas, valores medibles = `quantity`). El precio (`price`) vive en `pizzas`, no en `order_details`, así que para calcular ventas vas a necesitar relacionar tablas.

- **Se aplanó el esquema copo de nieve**: `pizza_types` se fusionó dentro de `pizzas` (Merge por `pizza_type_id`), quedando una sola dimensión `Pizzas` con tamaño, precio, nombre, categoría e ingredientes. Así se evita el esquema copo de nieve y se simplifica el filtrado.
- **Se usó `category` en vez de región**: el dataset no tiene datos de clientes ni geografía, así que `Pizzas[category]` (Classic, Chicken, Supreme, Veggie) se usó como dimensión de segmentación para el dashboard y para RLS.
